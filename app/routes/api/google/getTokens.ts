    ...
    try {
        ...
        const googleCalendarsPromise = googleCalendarAPI.calendarList
            ...
            .catch((error) => {
                logError({
                    filePath: '/api/google/getTokens',
                    message: `gogole error ~ googleCalendarAPI.calendarList.list()`,
                    error,
                });
        ...
        if (!googleMediciCalendar?.id) {
            const googleMediciCalendar = await googleCalendarAPI.calendars
                ...
                .catch((error) => {
                    logError({
                        filePath: '/api/google/getTokens',
                        message: `google error ~ googleCalendarAPI.calendars.insert()`,
                        error,
                    });
             ...
             */
            const dbMediciCalendar = user?.doctor?.googleData?.calendars.find(
                ({ googleCalendarId }) =>
        ...
        session.set(SessionData.GOOGLE_REFRESH_TOKEN, tokens.refresh_token);
